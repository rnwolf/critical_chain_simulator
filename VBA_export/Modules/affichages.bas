...
Public Function affichage_GANTT(t_triees As Collection)
    ' This function will display a Gantt chart.
    ' affichage = display
    ...
    'Clean of the old GANTT and graphs
    Call GANTT_clear
    ...
    ' Save the position of the critical buffer
    ...
    For i = 1 To t_triees.Count
        ...
        'If i <> 1 Then
            ...
            If t_triees(i).get_preds <> "" Then
                ...
                Next k
            ElseIf t_triees(i).get_type = 3 Then ' Tra�age des fl�ches pour les t�ches bleues
                'MsgBox "In it"
    ...
    s.Range("P2") = s.Cells(4, ThisWorkbook.Worksheets("LOGS").Cells(2, 1).value).value 'date de fin estim�e
    Call create_charts
...
Sub creer_calendrier(t_triees)
        'create_calendar
        ' We work 8 hours 5 days a week (working days)
        ' In the calendar we will skip the weekends so that the end date is realistic
...
'displaying a graph, title as a parameter
Sub add_chart(n As String)
    ...
    With ThisWorkbook.Worksheets("DASHBOARD")
        ...
        With .ChartObjects(.ChartObjects.Count).Chart
            'defininf title
            .HasTitle = True
            .ChartTitle.Text = "Cha�ne : " + n

            ' Definition of the colored areas
            ...
            .Axes(xlCategory).AxisTitle.Text = "% avancement de la cha�ne"
...
Sub create_charts()
    ...
    With ThisWorkbook.Worksheets("DASHBOARD")
        ...
        Dim i As Integer: i = 16 'We start after the critical chain
